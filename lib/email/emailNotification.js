...
module.exports = {
  sendAccoutnConfirmation: async (to, variables) => {
    const emailSender = setDefaults('sendgrid', to, activationTemplate, variables);
    await emailSender.send();
    return true;
  },
  sendResetPassword: async (to, variables) => {
    const emailSender = setDefaults('sendgrid', to, resetPasswordTemplate, variables);
    await emailSender.send();
    return true;
  },
  sendShareByEmail: async (to, variables) => {
    const emailSender = setDefaults('sendgrid', to, shareByEmailTemplate, variables);
    await emailSender.send();
    return true;
  },
};
